...
namespace MadreseManApi.Controllers
{
    ...
    public class ClassController : ControllerBase
    {
        ...
        public ClassController(DataContext context, AuthContext authContext)
        {
            _context = context;
        ...
        public IActionResult Get(string? session)
        {
            ...
            {
                return Ok(_context.classes.ToArray());
            }
            else
            {
        ...
        [HttpPost("Add")]
        public IActionResult Create([FromBody] Class entity, string? session)
        {
            if (session == null)
            {
                return BadRequest("{\"error\":\"لطفا ایدی سشن را ارسال کنید\"}");
        ...
        public IActionResult Delete(int? id, string? session)
        {
            ...
            else
            {
                if (_authContext.sessions.FirstOrDefault(e => e.SessionId == session) != null)
                {
                    Class entity = _context.classes.FirstOrDefault(e => e.id == id);
                    if (entity != null)
                    {
