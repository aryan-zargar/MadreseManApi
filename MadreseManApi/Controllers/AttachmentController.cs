...
namespace MadreseManApi.Controllers
{
    ...
    public class AttachmentController : ControllerBase
    {
        ...
        public AttachmentController(DataContext context, AuthContext authContext)
        {
            _context = context;
        ...
        public IActionResult Get(string? session)
        {
            ...
            {
                return Ok(_context.attachments.ToArray());
            }
            else
            {
        ...
        [HttpPost("Add")]
        public IActionResult Create([FromBody] Attachment entity, string? session)
        {
            if (session == null)
            {
                return BadRequest("{\"error\":\"لطفا ایدی سشن را ارسال کنید\"}");
        ...
        [HttpPut("Update")]
        public IActionResult Edit(string? session, [FromBody] Attachment newEntity)
        {
            if (session == null)
            {
                return BadRequest("{\"error\":\"لطفا ایدی سشن را ارسال کنید\"}");
            ...
            if (_authContext.sessions.FirstOrDefault(e => e.SessionId == session) != null)
            {
                Attachment entity = _context.attachments.FirstOrDefault(e => e.id == newEntity.id);
        ...
        public IActionResult Delete(int? id, string? session)
        {
            ...
            else
            {
                if (_authContext.sessions.FirstOrDefault(e => e.SessionId == session) != null)
                {
                    Attachment entity = _context.attachments.FirstOrDefault(e => e.id == id);
                    if (entity != null)
                    {
