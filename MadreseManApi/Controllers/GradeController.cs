...
namespace MadreseManApi.Controllers
{
    ...
    public class GradeController : ControllerBase
    {
        ...
        public GradeController(DataContext context, AuthContext authContext)
        {
            _context = context;
        ...
        public IActionResult Get(string? session)
        {
            ...
            {
                return Ok(_context.grades.ToArray());
            }
            else
            {
        ...
        [HttpPost("Add")]
        public IActionResult Create([FromBody] Grade entity, string? session)
        {
            if (session == null)
            {
                return BadRequest("{\"error\":\"لطفا ایدی سشن را ارسال کنید\"}");
        ...
        [HttpPut("Update")]
        public IActionResult Edit(string? session, [FromBody] Grade newEntity)
        {
            if (session == null)
            {
                return BadRequest("{\"error\":\"لطفا ایدی سشن را ارسال کنید\"}");
            ...
            if (_authContext.sessions.FirstOrDefault(e => e.SessionId == session) != null)
            {
                Grade entity = _context.grades.FirstOrDefault(e => e.id == newEntity.id);
                entity.grade_name = newEntity.grade_name;
        ...
        public IActionResult Delete(int? id, string? session)
        {
            ...
            else
            {
                if (_authContext.sessions.FirstOrDefault(e => e.SessionId == session) != null)
                {
                    Grade entity = _context.grades.FirstOrDefault(e => e.id == id);
                    if (entity != null)
                    {
                        ...
                        {
                            return BadRequest("{\"error\":\"به این پایه به کلاس یا کلاس هایی مربوط است. ابتدا آنها هارا حذف کنید\"}");
                        }
