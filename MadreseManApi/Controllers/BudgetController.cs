...
namespace MadreseManApi.Controllers
{
    ...
    public class BudgetController : ControllerBase
    {
        ...
        public BudgetController(DataContext context, AuthContext authContext)
        {
            _context = context;
        ...
        public IActionResult Get(string? session)
        {
            ...
            {
                return Ok(_context.budgets.ToArray());
            }
            else
            {
        ...
        [HttpPost("Add")]
        public IActionResult Create([FromBody] Budget entity, string? session)
        {
            if (session == null)
            {
                return BadRequest("{\"error\":\"لطفا ایدی سشن را ارسال کنید\"}");
        ...
        public IActionResult Delete(int? id, string? session)
        {
            ...
            else
            {
                if (_authContext.sessions.FirstOrDefault(e => e.SessionId == session) != null)
                {
                    Budget entity = _context.budgets.FirstOrDefault(e => e.id == id);
                    if (entity != null)
                    {
